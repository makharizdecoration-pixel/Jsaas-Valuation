...
interface EquipmentImageSwiperProps {
  images: string
  cardWidth?: number
  cardHeight?: number
  className?: string
...
export const EquipmentImageSwiper: React.FC<EquipmentImageSwiperProps> = ({
  images,
  cardWidth = 320,
  cardHeight = 400,
  className = "",
  ...
  }, [getDurationFromCSS, getActiveCard, applySwipeStyles, autoAdvance, autoAdvanceDelay])
  ...
  }, [cardOrder, autoAdvance, autoAdvanceDelay])
      ...
      {cardOrder.map((originalIndex, displayIndex) => (
        <article
          ...
          className="image-card absolute cursor-grab active:cursor-grabbing
                     place-self-center border border-zinc-600 rounded-xl
        ...
        >
          <img
            src={imageList[originalIndex] || "/placeholder.svg"}
            alt={`Equipment image ${originalIndex + 1}`}
            className="w-full h-full object-cover select-none pointer-events-none"
            draggable={false}
            loading="lazy"
          />
          <div className="absolute inset-0 bg-gradient-to-t from-zinc-900/60 via-transparent to-transparent opacity-0 hover:opacity-100 transition-opacity duration-300">
